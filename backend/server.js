...
app.get("/rating/:itemId", (req, res) => {
  ...
  db.query(
    "SELECT rating FROM itemRatings WHERE itemId = ?",
    [itemId],
    (err, result) => {
      if (err) {
...
app.post("/rating/:itemId", (req, res) => {
  ...
  db.query(
    "INSERT INTO itemRatings (itemId, rating) VALUES (?, ?) ON DUPLICATE KEY UPDATE rating = ?",
...
app.delete("/items/:id", (req, res) => {
  ...
  db.query(
    "DELETE FROM metalcore_merch WHERE id = ?",
    [itemId],
    (err, result) => {
      if (err) {
...
app.put("/items/:id", (req, res) => {
  ...
  db.query(
    "UPDATE metalcore_merch SET price = ? WHERE id = ?",
    [price, id],
    (err, result) => {
      if (err) {
...
app.post("/items", (req, res) => {
  ...
  const query =
    "INSERT INTO metalcore_merch (name, price, image, rating) VALUES (?, ?, ?, ?)";
  ...
  db.query(query, [name, price, image, rating], (err, result) => {
    ...
    res
      .status(201)
      .json({ success: true, message: "New item added.", id: result.insertId });
  });
});
